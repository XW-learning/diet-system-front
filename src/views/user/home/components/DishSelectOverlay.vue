    ...
    <div class="dish-select-overlay">
        <div class="search-header">
            ...
            <div class="meal-selector">
                <el-select v-model="currentMealType" class="meal-select" placeholder="选择餐次">
                    <el-option :value="1" label="🌞 早餐" />
                    <el-option :value="2" label="☀️ 午餐" />
            ...
            <div class="search-box">
                ...
                <input v-model="keyword" placeholder="搜索食物，如：苹果" @input="handleSearch" />
            </div>
        </div>
            ...
            <div class="dish-list">
                <div class="dish-card" v-for="dish in dishes" :key="dish.id">
                    <div class="dish-info">
                        ...
                        <div class="dish-cal">
                            <span class="highlight">{{ dish.calorie }}</span> 千卡 / {{ dish.refWeight }}{{
                            dish.weightUnit }}
                        </div>
                        <div class="dish-macros">
                            碳水 {{ dish.carbohydrate }}g | 蛋白 {{ dish.protein }}g | 脂肪 {{ dish.fat }}g
                        </div>
                    </div>
                    <div class="add-btn" @click="addToCart(dish, $event)">+</div>
                </div>

                <div v-if="loading" class="empty-state">加载中...</div>
                <div v-else-if="dishes.length === 0" class="empty-state">没有找到相关食物~</div>
            </div>
        </div>
        ...
        <div class="bottom-cart-bar-v2" :class="{ 'is-active': showCart, 'cart-bump': cartIsBumping }">
            <div class="cart-content-wrapper" @click="handleCartClick">
                <div class="cart-summary-left" ref="cartTargetRef">
                    <div class="food-count">已选 <span class="count-num">{{ cart.length }}</span> 种食物</div>
                    <div class="total-cal-v2">共 <span class="cal-num-v2">{{ totalCalorie }}</span> 千卡</div>
                </div>
        ...
        <SelectedFoodCart :visible="showCart" :cart="cart" @close="showCart = false" @update-weight="updateWeight"
...
import { doMealCheckInBatch } from '@/api/checkin';
...
const props = defineProps<{ defaultMealType?: number }>();

const currentMealType = ref(props.defaultMealType || 2);
const keyword = ref('');
...
interface CartItem { dish: DishVO; weight: number; }
...
const loadCategories = async () => {
    try {
        ...
        const listData = res.data || res; // 兼容不同拦截器
...
const fetchDishes = async () => {
    ...
    try {
        ...
        const res = await getDishList(requestData);
        const listData = res.data || res;
        if (Array.isArray(listData)) {
            dishes.value = listData;
        }
    } catch (err) {
        toastRef.value?.show('查询菜品失败');
    } finally {
        loading.value = false;
    }
};
...
const addToCart = (dish: DishVO, event: MouseEvent) => {
    const refWeight = 100;
    const weightUnit = '克';

    const existing = cart.value.find(item => item.dish.id === dish.id);
    if (existing) {
        existing.weight += refWeight;
    } else {
        cart.value.push({ dish: { ...dish, refWeight, weightUnit }, weight: refWeight });
    }
...
const totalCalorie = computed(() => {
    return cart.value.reduce((sum, item) => {
        const cal = (item.dish.calorie * item.weight) / item.dish.refWeight;
        return sum + Math.round(cal);
    }, 0);
});

const updateWeight = (dishId: number, newWeight: number) => {
    const item = cart.value.find(i => i.dish.id === dishId);
    if (item) item.weight = newWeight;
};

const removeFromCart = (dishId: number) => {
    cart.value = cart.value.filter(i => i.dish.id !== dishId);
...
const submitCheckIn = async () => {
    if (cart.value.length === 0) { toastRef.value?.show('请先选择食物'); return; }
    ...
    try {
        const batchData: MealCheckInDTO[] = cart.value.map(item => ({
        ...
        await doMealCheckInBatch(batchData);
...
.meal-select :deep(.el-select) {
    --el-select-border-color-hover: #66BB6A;
    --el-select-border-color-focus: #66BB6A;
}

.meal-select :deep(.el-select .el-input__wrapper) {
    background: linear-gradient(135deg, #FFFFFF 0%, #FFF9EA 100%);
    border: 2px solid #FFE4B5;
    border-radius: 16px;
    box-shadow: 0 2px 8px rgba(255, 228, 181, 0.25);
    padding: 2px 12px;
    transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
}

.meal-select :deep(.el-select:hover .el-input__wrapper) {
    border-color: #66BB6A;
    box-shadow: 0 4px 12px rgba(102, 187, 106, 0.2);
    background: linear-gradient(135deg, #FFFFFF 0%, #F8FFF8 100%);
}

.meal-select :deep(.el-select.is-focus .el-input__wrapper) {
    border-color: #66BB6A;
    box-shadow: 0 0 0 3px rgba(102, 187, 106, 0.15), 0 4px 12px rgba(102, 187, 106, 0.2);
    background: linear-gradient(135deg, #FFFFFF 0%, #F0FFF0 100%);
}

.meal-select :deep(.el-select .el-input__inner) {
    font-size: 14px;
    font-weight: 600;
    color: #7D5A44;
}

.meal-select :deep(.el-select .el-input__suffix) {
    color: #66BB6A;
}

.meal-select :deep(.el-select-dropdown) {
    border-radius: 16px;
    border: 2px solid #FFE4B5;
...
.meal-select :deep(.el-select-dropdown__item) {
    font-size: 14px;
    font-weight: 500;
    color: #7D5A44;
    border-radius: 12px;
...
.meal-select :deep(.el-select-dropdown__item:hover) {
    background: linear-gradient(135deg, #FFF9EA 0%, #FFF5E6 100%);
}

.meal-select :deep(.el-select-dropdown__item.is-selected) {
    background: linear-gradient(135deg, #66BB6A 0%, #81C784 100%);
    color: #FFFFFF;
    font-weight: 600;
}

.meal-select :deep(.el-select-dropdown__item.is-selected:hover) {
    background: linear-gradient(135deg, #66BB6A 0%, #81C784 100%);
    color: #FFFFFF;
}
